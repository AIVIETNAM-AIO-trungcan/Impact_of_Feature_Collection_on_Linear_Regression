...
def train_baseline_model(
    df_train, df_test, target_column="amount", model_name="baseline_model.pkl"
):
    ...
    error = pd.DataFrame({"actual": y_test, "pred": y_pred})

    error["abs_error"] = (error["actual"] - error["pred"]).abs()

    print(error.sort_values("abs_error", ascending=False).head(20))
